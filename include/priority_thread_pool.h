 ...
 * of Barak Shashany's thread_pool.hpp library.
 * It uses a priority queue of tasks
 * though note that the locking
 * of the priority queue will stronly disfavour
 * the use of many small tasks
 */
...
private:
    ...
    static inline bool
    compare_task_priority(const std::pair<int, std::function<void()>> &a,
    ...
    std::priority_queue<std::pair<int, std::function<void()>>,
                        ...
                        decltype(&compare_task_priority)>
        tasks;

    /**
     * @brief A variable to keep track of the total number of unfinished tasks -
